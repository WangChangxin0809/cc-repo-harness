...
in `$CLAUDE_CONFIG_DIR/cc-repo-harness/trusted-guards.json` (default `~/.claude`).
...
def store_path():
    ...
    return os.path.join(cfg, "cc-repo-harness", "trusted-guards.json")
...
NOT_TRUSTED = """\
cc-repo-harness: {root} has guards in scripts/guards/ that are NOT running.
...
def hook(raw, root):
    ...
    try:
        ...
        print(f"cc-repo-harness: could not run repo guards: {exc}", file=sys.stderr)
        return 0
