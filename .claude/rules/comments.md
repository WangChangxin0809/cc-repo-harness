...
This tree comments densely on purpose, and that is exactly why the bar has to
be stated: where every file carries reasoning, a restatement hides in the
crowd. Two failures, and only one of them looks like a mistake while you are
writing it.
...
**Excessive.** The comment is true, load-bearing somewhere, and not here —
the same argument made three times in one file, a paragraph of history above
a two-line helper, the decision record pasted in rather than pointed at.
