...
| `skills/bootstrap-repo-harness/` | the one skill that stays in the plugin: how a person arrives | every session on the machine, ~39 tokens a turn (0024); the whole plugin is capped at 171 by `check_plugin_structure.py` |
| `agents/` | `assess/`: the reader and the blind promise tester; `wiki/`: the maintainer, and the proposer that may open a pull request and may not merge one | spawned by `commands/assess.md`; they read a run and write answers or a reading, never the repository |
...
| `shared/wiki/` **payload, copied once** | the seed of `.claude/wiki/`: the catalogue, the run log, the impact table. `/learn` fills `patterns/` from the transcripts on the machine that runs it; no agent is routed there, because one that reads its own failure catalogue does worse (0059) | `check_wiki_hygiene.py`; the maintainer and the proposer |
...
| `shared/scripts/wiki/` **run from here** | `extract.py` turns the machine's transcripts of a repository into a digest of refusals, declines, errors, stops and user messages, redacted; `replay.py` runs its guards and any candidate over every recorded tool call, the held-out set a single repository does have (0059) | `commands/learn.md`, the `wiki-maintainer` agent, and the `wiki-proposer` that reads `--match` and `--candidate` |
