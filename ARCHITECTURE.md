...
| `skills/bootstrap-repo-harness/` | the one skill that stays in the plugin: how a person arrives | every session on the machine, ~39 tokens a turn (0024); the whole plugin is capped at 150 by `check_plugin_structure.py` |
| `agents/` | `assess/`: the reader and the blind promise tester; `wiki/`: the maintainer | spawned by `commands/assess.md`; they read a run and write answers or a reading, never the repository |
