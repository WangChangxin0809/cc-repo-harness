...
- A tool only *we* need does not belong in a copied directory, however
  convenient it is. `check_plugin_structure.py` sat in `gates/` for a while,
  shipping commented out of every `ci.sh`; it now runs from here and carries
  its own selftest, because the gates' harness only knows `gates/` -> 0058
