...
def probe(root):
    ...
    skills = []
    for base in (os.path.join(root, ".claude", "skills"),
        ...
        for name in sorted(os.listdir(base)):
            ...
            skills.append({
                "dir": os.path.relpath(os.path.join(base, name), root),
    ...
    r["always_on_skill_tokens"] = int(sum(s["desc_words"] for s in skills) * 1.35)
    ...
    r["discipline"].update({
        ...
        "retrieval": os.path.exists(os.path.join(root, "scripts", "index",
                                                 "build.py")),
    })
...
def render(r):
    ...
    out += [
        ...
        f"  gates / guards         {d['gates']} / {d['guards']}   selftests: {d['selftests']}",
        f"  CI entry               {', '.join(d['ci_entry']) or 'none found'}",
        f"  retrieval layer        {'present' if d['retrieval'] else 'absent'}",
